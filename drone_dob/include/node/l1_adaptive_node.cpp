...
void L1AdaptiveNode::dob_estimate()
{
    ...
    // Reset L1 state on airborne → ground transition.
    if(prev_in_flight_ && !in_flight_) l1_adaptive_model_->reset();
...
void L1AdaptiveNode::configure_parameters()
{
    ...
    this->declare_parameter<double>("l1_adaptive.lin_vel_filter_cutoff_hz", 0.0);
    lin_vel_filter_cutoff_hz_ = this->get_parameter("l1_adaptive.lin_vel_filter_cutoff_hz").get_value<double>();
